...
class ApiClient {
  ...
  private async request<T>(method: string, path: string, body?: unknown, query?: Record<string, string | number | undefined>): Promise<T> {
    // Use string concatenation to preserve the full base URL path (/api/v1)
    // new URL(path, base) treats paths starting with '/' as origin-relative,
    // which would incorrectly strip /api/v1 from the URL
